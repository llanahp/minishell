...
t_list	*save_pipe(t_inf *info, t_list *tmp, int pipe)
{
	...
	if ((command->previous == NULL && command->next== NULL && command->cmd == NULL)
		|| (command->previous != NULL && command->pipe_out == 1 && command->cmd == NULL && command->next== NULL && command->cmd == NULL))
	{
